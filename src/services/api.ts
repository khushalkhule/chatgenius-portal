  ...
  subscription: {
    ...
    getUserSubscription: async (userId) => {
      try {
        ...
        if (storedSubscription) {
          ...
          const planInfo = await api.subscription.getPlanById(subscription.planId);
          return {
            ...subscription,
            planInfo
          };
        ...
        const defaultPlan = await api.subscription.getPlanById("1");
    ...
    updateUserSubscription: async (userId, planId) => {
      try {
        ...
        // Fallback to localStorage
        const planInfo = await api.subscription.getPlanById(planId);
    ...
    getSubscription: async () => {
      try {
        ...
        const subscription = await api.subscription.getUserSubscription(userId);
...
export const initializeDatabase = async () => {
  try {
    ...
    if (!localStorage.getItem('subscriptionPlans')) {
      const mockPlans = await api.subscription.getAllPlans();
